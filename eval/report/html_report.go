...
func main() {
	...
	fmt.Println("<table><thead><tr><th>Case</th><th>Status</th><th>Weighted</th><th>Tool Precision</th><th>Tool Recall</th><th>Safety</th><th>Latency</th><th>Tags</th><th>Failure</th></tr></thead><tbody>")
	for _, result := range report.Results {
		...
		fmt.Printf("<tr><td>%s</td><td class=\"%s\">%s</td><td>%.2f</td><td>%.2f</td><td>%.2f</td><td>%t</td><td>%dms</td><td>%s</td><td>%s</td></tr>", html.EscapeString(result.CaseID), className, status, result.TaskCompletion.WeightedScore, result.ToolCorrectness, result.ToolRecall, result.SafetyCompliance, result.LatencyMs, html.EscapeString(fmt.Sprint(result.Tags)), html.EscapeString(result.FailureReason))
	}
